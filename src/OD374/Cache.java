...
public class Cache {
    //披萨
    static int[] pizza;
    //缓存 记录当披萨缺口为left right时遍历能吃到的最多披萨量
    static long[][] cache;
    ...
    public static long getMaxOfPizza(int left, int right) {
        ...
        //cache默认是为0，如果有值，则说明这种情况已经递归得到过，直接返回
        ...
        if (left == right) {
            ...
            //如果还剩多块披萨，缓存记录当前情况下的最大值
            cache[left][right] = Math.max(getMaxOfPizza(check(left - 1), right) + pizza[left], getMaxOfPizza(left, check(right + 1)) + pizza[right]);
        }
        return cache[left][right];
